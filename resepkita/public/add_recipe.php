...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '') {
        ...
        try {
            ...
            // ingredients field store comma-separated names for quick display
            // we'll build it from selected + new
            // handle new ingredients: insert them first (ignore existing)
            $new_ids = [];
            if ($new_ings_raw !== '') {
            ...
            if (!empty($ingredient_ids)) {
                ...
                // prepare select to fetch their names
            ...
            // add new parts names to display list
            if ($new_ings_raw !== '') {
                $parts = array_filter(array_map('trim', explode(',', $new_ings_raw)));
            ...
            $stmt->bind_param('sss', $title, $ingredients_display, $instructions);
            $stmt->execute();
            ...
            // new names -> insert ignore then select id
            if ($new_ings_raw !== '') {
                $parts = array_filter(array_map('trim', explode(',', $new_ings_raw)));
            ...
            // refresh ingredient list in form (to include new ones)
            $ings = [];
            $res = $mysqli->query("SELECT id, name FROM ingredients ORDER BY name ASC");
            while ($r = $res->fetch_assoc()) { $ings[] = $r; }
            $res->free();
